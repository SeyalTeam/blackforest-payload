...
export const toggleBranchClosingAccessHandler: PayloadHandler = async (req): Promise<Response> => {
  ...
  try {
    ...
    const enabled = body?.enabled !== undefined ? Boolean(body.enabled) : (body?.isClosingEntryEnabled !== undefined ? Boolean(body.isClosingEntryEnabled) : true)
    ...
    // Update branch closing access
    const updatedBranch = await req.payload.update({
      collection: 'branches',
      id: branchId,
      data: {
        isClosingEntryEnabled: enabled,
      } as any,
      overrideAccess: true,
    })
    ...
    return Response.json({
      success: true,
      message: `Branch closing entry access ${enabled ? 'enabled' : 'locked'} successfully`,
      branchId,
      isClosingEntryEnabled: updatedBranch.isClosingEntryEnabled,
    })
  } catch (error: any) {
    req.payload.logger.error({
      err: error,
      msg: 'Failed to toggle branch closing access',
    })
