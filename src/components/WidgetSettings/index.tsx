...
const WidgetSettings: React.FC<any> = (props) => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        const [branchesResponse, settingsResponse, appDownloadsResponse, waitersResponse] = await Promise.all([
          ...
          fetch('/api/users?limit=1000&depth=0&where[role][in][0]=waiter&where[role][in][1]=supervisor&where[role][in][2]=cashier&where[role][in][3]=manager'),
        ])
  ...
  const getWaiterOptionsForTable = (
    ...
    const options = liveLoginUsers
      .filter((u) => {
        if (!['waiter', 'supervisor', 'cashier', 'manager'].includes(u.role)) return false
  ...
  const fetchBranchWaiters = async (branchId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/users?limit=1000&depth=0&where[role][in][0]=waiter&where[role][in][1]=supervisor&where[role][in][2]=cashier&where[role][in][3]=manager&where[lastLoginBranch][equals]=${branchId}`
      )
      const json = await response.json()
