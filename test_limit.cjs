...
const options = {
  ...
  path: '/api/billings?limit=1000&depth=1',
...
https.get(options, (res) => {
  ...
  res.on('end', () => console.log('Status:', res.statusCode, 'Total fetched:', JSON.parse(data).docs.length, 'Total in DB:', JSON.parse(data).totalDocs));
});
