# API Compatibility Matrix (Phase 0)
...
|---|---|---|---|---|---|
| REST base `https://blackforest.vseyal.com/api/v1` | Current routes are mounted under `/api` | Partial | `/api/v1` namespace/adaptor layer not present | Backend | Phase 2 |
...
| `GET /billings` | `GET /api/billings` | Partial | Query style today is Payload-style filtering, not frozen draft query params | Backend | Phase 2 |
| `GET /billings/{billId}` | `GET /api/billings/{id}?depth=...` | Partial | Envelope + path versioning only | Backend | Phase 2 |
...
| `POST /billings/{billId}/waiter-calls/{callId}/ack` | `POST /api/call-waiter/ack` | Partial | Path, payload, and waiter-call identity model differ | Backend | Phase 2 |
...
| Draft Endpoint | Current Live Equivalent | Status | Gap | Owner | ETA |
|---|---|---|---|---|---|
| `GET /branches` | `GET /api/branches` | Partial | Envelope + `/api/v1` adapter missing | Backend | Phase 2 |
| `GET /branches/{branchId}` | `GET /api/branches/{id}` | Partial | Envelope + `/api/v1` adapter missing | Backend | Phase 2 |
...
| `GET /companies/{companyId}` | `GET /api/companies/{id}` | Partial | Envelope + `/api/v1` adapter missing | Backend | Phase 2 |
